...
function get_code_url(url,path){
	path = path.replace('/','')
	const file = url.lastIndexOf('/') === url.length-1 ? `${url}${path}.js`:`${url}/${path}.js`
	return fxy.source.join(url,path)
}