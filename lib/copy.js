...
function get_modules(public_directory) {
	return new Promise((success, error) => {
		...
		if (!fxy.exists(modules_directory)) return create_modules_folder(modules_directory).then(success).catch(error)
		return success(modules_directory)
	})
}

function get_wwi_copy(modules_directory,logs) {
	return new Promise((success, error) => {
		let wwi_directory = fxy.join(modules_directory, 'wwi')
		//console.log(wwi_directory, 'exists: ' + fxy.exists(wwi_directory))
		if(fxy.exists(wwi_directory)){
			try{ clean_folder(wwi_directory,logs) }
			catch(e){ return error(e) }
		}
		if (!fxy.exists(wwi_directory)) {
			if(logs !== false) console.info(`Creating "wwi" directory at:${wwi_directory}`)
			fxy.mkdirp.sync(wwi_directory)
			return fxy.copy_dir(wwi, wwi_directory)
			          .then(copier => {
...
function install_wwi(directory,logs) {
	if(logs !== false) console.info('Installing "wwi" at: ', directory)
	if (fxy.exists(directory)) {
		return get_modules(directory).then(modules_directory => {
			return get_wwi_copy(modules_directory,logs)
		})
	}
