...
(function(){
    return (function export_library(){
				    ...
				    return new Promise((success,error)=>{
					    return window.app.port(element_data.path, {'from-library': library.name})
					                 .then(()=>{
						                 ...
						                 return window.fxy.when(...element_data.wait).then(()=>success(element_data))
					                 })
					                 .catch(e=>{
	    ...
	    function load_ports(library) {
		    return get_files('ports' in library ? library.ports:[])
		    function get_files(files){
			    let count = files.length
			    let loaded = 0
			    return new Promise((success, error)=>{
		    ...
		    function get_port(item) {
		    	return new Promise((success,error)=>{
				    item = fxy.is.text(item) ? {path:item}:item
				    if(fxy.is.data(item) && 'path' in item && fxy.is.text(item.path)){
					    let file_url = item.path.includes('http') ? item.path:window.url(item.path)
					    return window.app.port(file_url).then(()=>success(file_url)).catch(error)
				    }
				    else console.warn('Ignored port item for library -> ',item)
				    return success()
			    })
		    }
	    }
