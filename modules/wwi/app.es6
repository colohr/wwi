(function(root, init){return init(root)}(this, function(window){return (function (root, factory, bootstrap, factories){return factory(root, bootstrap, factories)})(window,
	...
	function bootstrap(window, Factories) {
		...
		return window.app = new Proxy(window.application,{
			...
			ownKeys(o) {return Object.keys(o)},
			set(o,name,value){
				o[name] = value
				return true
			}
	...
	function factories(window){return {
				CreateSupers( Helpers ){
					...
					class SuperString extends String {
						static context(v) {
							...
							switch (type) {
								case 'object':
									let otype = get_object_string_type(v)
									kind = otype.kind
									v = otype.string
									break
								case 'function':
					...
					class QueryParams extends SuperString {
						...
						static parse(v) {
							...
							try{ return JSON.parse(v) }catch (e) { console.error(e) }
							return v
						}
							...
							if (this.kind === 'json') this.QueryData = this.context.origin
								...
								if (typeof this.QueryData === 'undefined') {
									if (typeof this.kind === 'string') {
										...
										if(typeof this.QueryData === 'undefined') this.QueryData = Helpers.query(this.queryString)
									}
								}
							}
							if(typeof this.QueryData === 'object'){
								for(let key in this.QueryData){
									if (typeof this.QueryData[key] === 'string'){
										let o = QueryParams.parse(this.QueryData[key])
										if (typeof o === 'object') this.QueryData[key] = o
									}
								}
							}
						...
						get encoded(){return encodeURIComponent(this.queryString)}
						get kind(){return this.context.kind}
						get queryString(){
							if(typeof this.QueryString === 'undefined'){
								if (this.kind === 'string') this.QueryString = decodeURIComponent(this)
								else this.QueryString = this
							}
					...
					//return value
					return {SuperString, QueryParams}
					
					//shared actions
					function get_object_string_type(v){
						let kind, string = ''
						if( v === null ) kind = 'invalid'
						else if(v instanceof HTMLElement) { string = v.innerHTML; kind = 'html'; }
						else{
							try{ string = JSON.stringify(v); kind = 'json'; }
							catch(e){ string = ''; kind = 'invalid'; }
						}
						return {kind, string}
					}
				...
				CreateHelper(){
					...
					const style=string=>{
						let css = {}
						str.array(string,';').map(x=>str.array(x, ':')).map(function(x){return this[x[0]]=x[1]},css)
						return css
					}
					...
					const query=string=>{
						...
						str.array(string,'&').map(x=>str.array(x, '=')).map(function(x){return this[x[0]]=x[1]},qs)
						return qs
					}
