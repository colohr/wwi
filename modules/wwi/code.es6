...
function wwi_get_fxy(element, export_base, port_module, window){
	...
	//return value
	return get_modules().then(_=>export_base(base,port_module,window)).catch(console.error)
	
	//shared actions
	function get_base_kit(){
		base.kit = {
			...
			url(...x){ x.unshift(this.dir); return window.fxy.file.url( ...x ) }
		}
		return window.kit = base.kit
	}
	...
	function get_modules(){ return new Promise((success,error)=>get_fxy().then(_=>get_base_kit()).then(_=>get_url()).then(success).catch(error)) }
	function get_url(){ return window.fetch(`${base.host}/${base.modules}/url/url.es6`).then(response=>response.text()).then(url_source=>window.eval(url_source)(base.modules)) }
},
function wwi_get_kit(base,port_module,window){
	...
	function setup(){
		...
		if(base_element === null){
			...
			base_element.href = window.fxy.file.url(base.element.hasAttribute('path') ? base.element.getAttribute('path') :'/')
			document.head.insertBefore(base_element,base.element)
		}
		base.element.setAttribute('wwi-port-load','')
		base.base_element = base_element
		base.javascript = base.element.src.includes('es6') ? 'es6':'js'
		if(!base.kit.has('service-worker')) base.kit.set('service-worker',window.url.logic('service-worker.js'))
		return port_module(...[ base, base.element, window.fxy.dom.query, window.fxy.port, window.fxy.file, window ])
	}
	
},
function wwi_module(base, element, query, resource, source, window){
		...
		function window_resources(){
			...
			function resource_promise(item){
				return get_eval(item).then(response=>{
					...
					return response({ item, kit, port:{ base, eval: get_eval, resource } , source })
				})
			}
		}
	}
