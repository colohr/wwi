(function () {
	...
	(function (m) {
		...
		function v(a) {
			var b = document.querySelectorAll("link[rel=import]:not(import-dependency)"),
				c = b.length;
			if (c)for (var d = 0, e = b.length, h; d < e && (h = b[d]); d++)w(h, function () {--c || a()}); else a()
		}
		...
		function p(a) {q(function () {return v(function () {return a && a()})})}
		...
		function u(a) {return a.nodeType === Node.ELEMENT_NODE && "link" === a.localName && "import" === a.rel}
		
		function l() {
			var a = this;
			this.b = {};
			this.c = 0;
			this.h = new MutationObserver(function (b) {return a.C(b)});
		...
		var G = /(^\/)|(^#)|(^[\w-\d]*:)/, H = /(url\()([^)]*)(\))/g, I = /(@import[\s]+(?!url\())([^;]*)(;)/g,
			J = /(<link[^>]*)(rel=['|"]?stylesheet['|"]?[^>]*>)/g, f = {
				A: function (a, b) {
				...
				i: function (a, b) {return a && G.test(a) ? a : f.v(a, b)},
				v: function (a, b) {
					if (void 0 === f.g) {
						f.g = !1;
						try {
		...
		l.prototype.D = function (a, b) {
			...
			A && (a = a.replace(J,
				function (a, b, c) {return -1 === a.indexOf("type=") ? b + " type=import-disable " + c : a}));
		...
		l.prototype.F = function (a) {
			function b(e) {
				if (e < d) {
					...
					B = g;
					let id = h.hasAttribute('id') ? h.getAttribute('id'):h.getAttribute('src')
	...
	(function () {
		...
		if (m && m.polyfillWrapFlushCallback) {
			...
			n.whenReady = function (k) {q(function () {v() ? n.whenReady(k) : k()})}
		}
