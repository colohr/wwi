wwi.exports('element',(element,fxy)=>{
	...
	class Type extends Map{
		...
		trigger(original){
			...
			let event = { action, name, original, target }
			...
			if('activates' in this) set_activates( this, event )
	...
	class A11y extends Type{
		...
		add(e){
			...
			if(fxy.is.element(e)){
				...
				if(contains.tabs) e.setAttribute('tabindex', contains.tabindex)
		...
		dump(){
			if( this.has('contents') ){
				if(this.contents.size === 0) this.delete('contents')
			}
			return this
		}
	...
	function has_ally_actions(type){
		return a11y_actions in allies && allies.actions.has(type)
	}
...
//span.addEventListener("keypress", this, false);
...
/*
 //case 'activates':
 //	this.set('activator',new A11yActivator(this.name,data[name]))
 //	break
 //get activator(){ return this.get('activator') }
class A11yActivator extends A11yKind{
	constructor(activator, activates){
		super(activator)
		for(let name in activates){
			this.set(activates[name])
		}
	}
	get on(){ return this.has('on') ? this.get('on'):[]}
 ...
 //__add(e){
 //	if(e instanceof HTMLElement && this.has('contains')){
 //		let contains = this.contains
 //		e.setAttribute('role',contains.role)
 //		if(contains.tabs) e.setAttribute('tabindex',contains.tabindex)
 //
 //		if(this.has('content') && !this.contents.has(e)){
 //			let content = this.get('content')
 //			let binder = { type:contains, container:this.container   }
 //			if(this.has('activates')) binder.activates = this.get('activates')
 //			for(let key in content){
 //				e.addEventListener(key, this.trigger.bind(binder), false)
 //			}
 //			this.contents.add(e)
 //		}
 //	}
 //	return e
 //}
 //get name(){ return this.has('type') ? this.type.name:null }
 //get type(){
 //	return this.get('type')
 //}
 //get root(){ return this.get('container') }
*/