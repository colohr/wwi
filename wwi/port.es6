...
function wwi_port_base( port_module, window  ){
	...
	const source = {
		...
		type(name, ...custom_types){
			var valid_types = file.types.filter( type => { return name.includes(`.${type}`) })
			if(valid_types.length) return valid_types[0]
			if(custom_types.length){
				valid_types = custom_types.filter( type => { return name.includes(`.${type}`) })
			}
			return valid_types.length ? valid_types[0]:''
		},
		url( ...paths ){
	...
	function setup( base ){
		...
		if(base_element === null){
			...
			base_element.href = source.url(base.element.hasAttribute('path') ? base.element.getAttribute('path') :'/')
			document.head.insertBefore(base_element,base.element)
		}
...
function wwi_port_module( base, element, find, file, query, resource, source, window ){
	...
	const kit = {
		...
		get base(){
			...
			if(base_element === null){
				base_element = document.createElement('base')
				base_element.href = source.url(this.path)
				document.head.insertBefore(base_element,this.selfie.previousElementSibling)
			}
			return base_element
		},
	...
	function get_service_worker(){ return 'serviceWorker' in navigator && kit.has('service-worker') ? navigator.serviceWorker.register(kit.get('service-worker')):null }
	function get_window_url(){
		const window_url = new Proxy({
			folders:{
				bower:kit.has('bower') ? kit.get('bower') : 'bower_components',
				edus:'edus',
				elements:kit.has('elements') ? kit.get('elements'):'custom-elements',
				logic:kit.has('logic') ? kit.get('logic') : 'logic',
				modules:kit.modules || 'modules',
				site:'/',
			...
			has(o,name){ return name in o.folders }
		})
		
		return new Proxy(function get_url(...x){return window_url.site(...x)},{ get(o,name){ return window_url[name] } })
	}
	function window_load(){
		...
		function window_ports(resources){
			...
			//shared actions
			function finish(){
				return load_library().then(_=>load_extensions())
			...
			//return Promise.all(load_extensions())
			//              .then(_=>window_firebase())
			//              .then(_=>get_eval(window.url.library('library.es6')))
			//              .then(_=>get_eval(window.url.component('component.es6')))
			//              .then(window_imports)
		}
			...
			function promise_all(resources){
				...
				function promise_next(done,error){
					for(let name in sources){
						return resource_promise(sources[name]).then(_=>{
							delete sources[name]
							return promise_next(done,error)
						}).catch(error)
					}
					return done()
				}
...
//let design_css_url =window.url.component('design/css/index.css')
//let design_style = document.createElement('link')
//design_style.rel = 'stylesheet'
//design_style.href = design_css_url
//document.head.appendChild(design_style)